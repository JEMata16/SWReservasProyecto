...
const ReservationButton: React.FC<ReservationProps> = ({ id }) => {
  ...
  const mutation = api.affiliation.createReservation.useMutation();
  ...
  const makeReservation = async () => {
    ...
    try {
      await mutation.mutateAsync({
        ...
        userid: userId ?? '',
        phoneNumber: phone,
        email: userResponse.data.user.emailAddresses?.[0]?.emailAddress ?? '',
      });
      router.push('/');
    } catch (error) {
...
const AffiliateHotelDetails: React.FC<HotelDetailsProps> = ({ hotelId, userId }) => {
  const { data } = api.affiliation.getHotelById.useQuery({ text: hotelId })
  const hotel = {
    id: data?.id,
    name: data?.name,
...
export default AffiliateHotelDetails;
